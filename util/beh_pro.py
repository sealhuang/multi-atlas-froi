...
similarity = np.zeros((len(sessid), len(sessid)))
for i in range(len(sessid)):
    for j in range(i+1, len(sessid)):
        i_vtr = spatial_ptn[..., i].copy()
        i_vtr[i_vtr<2.3] = 0
        i_vtr[i_vtr>0] = 1
        j_vtr = spatial_ptn[..., j].copy()
        j_vtr[j_vtr<2.3] = 0
        j_vtr[j_vtr>0] = 1
        similarity[i, j] = normalized_mutual_info_score(i_vtr, j_vtr)
np.savetxt('nmi_similarity.txt', similarity, delimiter=',')
